...
import { EntityArrayResponseType, RefreshTokenService } from '../service/refresh-token.service';
...
export class RefreshTokenComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRefreshToken): number => this.refreshTokenService.getRefreshTokenIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.refreshTokens = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IRefreshToken[] | null): IRefreshToken[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.refreshTokenService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
