...
describe('Alert Error Component', () => {
  ...
  describe('Error Handling', () => {
    it('Should display an alert on status 0', () => {
      // GIVEN
      eventManager.broadcast({ name: 'dataRunClientApp.httpError', content: { status: 0 } });
      // THEN
    ...
    it('Should display an alert on status 404', () => {
      // GIVEN
      eventManager.broadcast({ name: 'dataRunClientApp.httpError', content: { status: 404 } });
      // THEN
      expect(comp.alerts().length).toBe(1);
    ...
    it('Should display an alert on generic error', () => {
      // GIVEN
      eventManager.broadcast({ name: 'dataRunClientApp.httpError', content: { error: { message: 'Error Message' } } });
      eventManager.broadcast({ name: 'dataRunClientApp.httpError', content: { error: 'Second Error Message' } });
      // THEN
    ...
    it('Should display an alert on status 400 for generic error', () => {
      ...
      const response = new HttpErrorResponse({
        ...
        error: {
          type: 'https://nmcpye.org/problem/constraint-violation',
          title: 'Bad Request',
          status: 400,
          path: '/api/foos',
      ...
      eventManager.broadcast({ name: 'dataRunClientApp.httpError', content: response });
      // THEN
      expect(comp.alerts().length).toBe(1);
